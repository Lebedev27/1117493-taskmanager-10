  ...
  return (
    `<section class="main__filter filter container">
        ...
        <label for="filter__all" class="filter__label">
          All <span class="filter__all-count">0</span></label
        >
        <input
          type="radio"
          id="filter__overdue"
...
const renderElements = () => {
  ...
  render(siteHeaderElement, createMenu(), `beforeend` );
  render(siteMainElement, createFilter(), `beforeend`);
  render(siteMainElement, createTaskList(), `beforeend`);

  const siteTaskListElement = siteMainElement.querySelector(`.board__tasks`);

  render(siteTaskListElement, createTaskEditor(), `beforeend`);

  new Array(CARD_COUNT).fill(``).forEach(
    () => render(siteTaskListElement, createTaskCard(), `beforeend`)
  );
